...
def convert_to_smallest(src, dest, tmpdir):
    ...
    if any(os.path.isfile(dest + root + target_ext) for target_ext in trial_exts[ext.lower()]):
        print(f"target for {src} already exists, skipping...")
        return
    trial_files = [convert(src, tmpdir + root + target_ext) for target_ext in trial_exts[ext.lower()]]
    trial_files += [src]
...
def optimize_directory(src, dest_prefix):
    with tempfile.TemporaryDirectory() as tmp:
        for file in glob.iglob(src + "/**/*.*", recursive=True):
            ...
            convert_to_smallest(file, dest_prefix+os.path.dirname(file) + "/", tmp + "/")
